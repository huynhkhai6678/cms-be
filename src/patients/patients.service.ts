...
export class PatientsService {
  ...
  async create(createPatientDto: CreatePatientDto, imageUrl: string) {
    ...
    const patientMedicalDto =
      this.patientMedicalRepository.create(createPatientDto);
  ...
  async findOne(id: number, clinicId: number) {
    ...
    if (user?.address?.country_id) {
      states = await this.helpService.getStateByCountry(
    ...
    const patient_unique_id = patient
      ? patient.patient_unique_id
      : await this.generatePatientUniqueId();
    const patient_mrn = patient
      ? patient.patient_mrn
      : await this.generatePatientMRN();
  ...
  async remove(id: number) {
    ...
    const patientMedical = await this.patientMedicalRepository.findOneBy({

...
export class BloodPressureController {
  constructor(private readonly bloodPressureService: BloodPressureService, private readonly medicalRecordService: MedicalRecordService) {}

  @Post()
  async create(@Body(ValidationPipe) createBloodPressureDto: CreateBloodPressureDto, @Req() request : any) {
    const bloodPressure = await this.bloodPressureService.create(createBloodPressureDto);
  ...
  @Patch(':id')
  update(@Param('id', ParseIntPipe) id: number, @Body(ValidationPipe) updateBloodPressureDto: UpdateBloodPressureDto) {

...
export class PulseRateController {
  constructor(private readonly pulseRateService: PulseRateService, private readonly medicalRecordService: MedicalRecordService) {}

  @Post()
  async create(@Body(ValidationPipe) createPulseRateDto: CreatePulseRateDto, @Req() request : any) {
    const pulseRate = await this.pulseRateService.create(createPulseRateDto);
    const data = `Pulse rate : ${pulseRate.pulse}`;
    await this.medicalRecordService.addHistory(request.user.id, pulseRate.patient_medical_record_id, data);
  }
  ...
  @Patch(':id')
  update(@Param('id', ParseIntPipe) id: number, @Body(ValidationPipe) updatePulseRateDto: UpdatePulseRateDto) {

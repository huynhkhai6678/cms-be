...
export class UsersService {
  constructor(
    ...
    @InjectRepository(ClinicChain) private clinicChainRepo: Repository<ClinicChain>,
    private database: DatabaseService,
  ) {}

  async create(createUserDto: CreateUserDto) {

    const count = await this.userRepository.countBy({ email : createUserDto.email });
    if (count > 0 ) {
    ...
    const userDto = this.userRepository.create({
      ...
      contact : createUserDto.phone.e164Number.split(createUserDto.phone.dialCode)[1], 
  ...
  async update(id: number, updateUserDto: UpdateUserDto) {
    ...
    user.contact = updateUserDto.phone.e164Number.split(updateUserDto.phone.dialCode)[1];

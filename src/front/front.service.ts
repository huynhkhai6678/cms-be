...
export class FrontService {
  constructor(
    ...
    private testimominalRepo: Repository<FrontPatientTestimonial>,
    @InjectRepository(Specialization)
    private specilizationRepo: Repository<Specialization>,
  ...
  async getServiceCounter(clinicId: number) {
    ...
    const specializationCount = await this.specilizationRepo.countBy({
      clinic_id: clinicId,
    });
  ...
  async getDoctors(clinic_id) {
    const doctors = await this.doctorRepo
      .createQueryBuilder('doctor')
      .innerJoinAndSelect('doctor.user', 'user', 'user.status = :status', {
        status: 1,
      })
      .innerJoin(
        'user.user_clinics',
